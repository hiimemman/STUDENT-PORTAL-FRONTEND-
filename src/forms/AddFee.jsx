...
import { CLOSEFEEFORM } from '../slice/AddFormSlice/AddFeeSlice/AddFeeSlice';
 ...
 console.log(courses)
  ...
  const handleChangeSubCode = async (event) =>{
    ...
    try{
      ...
      const sendRequest = await fetch(basedUrl+"/exist-fee-name.php",{
        method: "POST",
        body: data,
    });
...
//`action`,`category`,`editor_position`,`editor_email`,`edited_email`
  ...
  try{
    const sendRequest = await fetch(basedUrl+"/fee-add.php",{
              method: "POST",
              body: data,
          });
  ...
  return(
    ...
    {console.log(courses)}
      ...
      >
        ...
        <DialogTitle id="scroll-dialog-title">Add Fee</DialogTitle>
        ...
        <Grid2 container spacing={3} sx ={{marginLeft:'-10px'}}>
             <Grid2 item xs={12}>
              <FormControl fullWidth>
               <TextField error = {errorSubjectCode} name ="Name"  required label="Name" variant="outlined" onKeyUp =  {handleChangeSubCode} />
        {errorSubjectCode === true ? (<FormHelperText id="component-helper-text">Fee already exist
        </FormHelperText>): (<></>)}
               </FormControl>
             </Grid2>
             ...
             <Grid2 item xs={12}>
              <FormControl fullWidth>
               <TextField error = {errorUnits} type ="number" name ="Amount" fullWidth required label="Amount" variant="outlined" onKeyUp =  {handleChangeUnits} />
        {errorUnits === true ? (<FormHelperText id="component-helper-text">Amount must not be empty
        </FormHelperText>): (<></>)}
               </FormControl>
             </Grid2>
