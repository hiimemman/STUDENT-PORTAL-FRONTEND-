...
import {  Alert, Skeleton, Snackbar, Stack, Paper, Avatar, TextField, FormControl, FormHelperText, Divider, Container, Button} from '@mui/material';
...
return () =>{
  ...
  const handleSubmitChangePassword = async (event) =>{
    ...
    if(!errorCurrentPassword && !errorConfirmPassword && !errorNewPassword){
      const data = new FormData(event.target);
      data.append('StudentNumber', studentSession.studetnumber)
      for (var pair of data.entries()) {
        console.log(pair[0]+ ' - ' + pair[1]); 
    }
      try{
        isLoading(true);
        //online api
          const sendRequest = await fetch(basedUrl+"/student-change-confirmed-password.php",{
              method: "POST",
              body: data,
          });
...
const checkCurrentPassword = async(event) =>{
  ...
  try{
    ...
    const sendRequest = await fetch(basedUrl+"/student-check-password-correct.php",{
      method: "POST",
      body: data,
  });
...
'1.5rem' }} className="rounded-xl">
      ...
      <Grid2 container sx={{ paddingRight: 10, }}>
        ...
        <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField disabled defaultValue={studentSession.firstname} name="FirstName" id="FirstName" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
        ...
        <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField disabled defaultValue={studentSession.middlename} name="MiddleName" id="MiddleName" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
        ...
        <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField  disabled defaultValue={studentSession.lastname} name="LastName" id="LastName" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
 ...
 <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField disabled defaultValue={studentSession.birthday} type ="date" name="Birthday" id="Birthday" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
     ...
     <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField disabled defaultValue={studentSession.contact} name="Contact" id="Contact" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
        ...
        <Stack direction="row" spacing={2} sx={{ width: '100%', marginBottom: '1.5rem' }}>
          ...
          <FormControl fullWidth>
            <TextField disabled required defaultValue={studentSession.address} name="Address" id="Address" sx={{ fontSize: '15px' }} variant="standard" inputProps={{ 'aria-label': 'description' }} />
          </FormControl>
